...
new_input = Input(shape=(image_size, image_size, 3))
model = EfficientNetB7(include_top=False, input_shape=(image_size, image_size, 3), weights='imagenet')
...
tensorboard = TensorBoard(log_dir='logs/EfficientNetB7')

model.fit(train_x, train_y, epochs=10, batch_size=128, validation_split=0.1, callbacks=[tensorboard])

#model.summary()
model.save('models/efficientnetb7/')
...
new_input = Input(shape=(image_size, image_size, 3))
model = MobileNet(include_top=False, input_shape=(image_size, image_size, 3), weights='imagenet')
...
tensorboard = TensorBoard(log_dir='logs/MobileNet')
